...
def homepage():
    CountryName = request.form.get('Country_field','SUTTON')
    ...
    df = df[df.IncGeo_BoroughName == CountryName]
    df = df[df.CalYear == int(Year)]
    print(df.head())
    # df = df.drop(
    # ['Country', 'Item Code', 'Flag', 'Unit', 'Year Code', 'Element', 'Element Code', 'Code', 'Item'], axis=1)
    #df = df[["WardName", "Cat", "value"]]
    ...
    df1 = df.groupby(['WardName']).size().reset_index(name = 'Count')
    df1 = df1.reset_index()
    ...
    for line in df1.values:
        ...
        if not Category in keys_list:
            d['children'].append({"name": Category, "children": [{ "size": value}]})

        # if 'the_parent' IS a key in the flare.json, add a new child to it
        else:
            d['children'][keys_list.index(Category)]['children'].append({ "size": value})
   ...
   #         d['children'][keys_list.index(Category)]['children'].append({"name": Cat, "size": value})
    ...
    return render_template("histogramTest.html",CountryName=CountryName,Year=Year,Prod=Prod)
